...
def get_message_subjects(user: User):
    ...
    if latest_sync:
        search_str = f'after:{latest_sync} {{(subject:{{+subscribing +subscription}}) {{+subscribing +subscription}}}}  {{+price +order +cost +receipt +charge}} -{{newsletter}} -(special AROUND offer) -"claim your offer" -"manage your preferences" -"manage your emails" -"manage your subscription" -"unsubscribe from" -"unsubscribe AROUND manage" -promotion -subject:{{cancelled renew}}',
        print(latest_sync)
    else:
